...
public abstract class MouseAStar extends MouseMovement {

	protected ArrayList<Tile> AStarSearch(Tile position, Tile target) {
		...
		// heuristic.
		PriorityQueue<AbstractMap.SimpleEntry<Tile, Integer>> fScore = new PriorityQueue<AbstractMap.SimpleEntry<Tile, Integer>>();
		// For the first node, that value is completely heuristic.
		fScore.add(new AbstractMap.SimpleEntry<Tile, Integer>(position,
		...
		while (!openSet.isEmpty()) {
			...
			for (Tile neighbour : current.neighbours()) {
				if (!closedSet.contains(neighbour)) {
					...
					fScore.add(new AbstractMap.SimpleEntry<Tile, Integer>(neighbour,
	...
	private Integer moveToNeighbour(Tile neighbour) {
		return 1;
	}

	private Integer manhattanDistance(Tile position, Tile target) {
		return Math.abs(position.getX() - target.getX()) + Math.abs(position.getY() - target.getY());
	}

	private ArrayList<Tile> getPath(Hashtable<Tile, Tile> cameFrom, Tile current) {
